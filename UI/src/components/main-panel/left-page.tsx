...
import { lookAt, setToolStateCmd, useGetSelectedEdgeEntityCmd, useGetToolStateCmd } from 'hooks/cmd'
        ...
        <Scrollable>
          {
            slectedEdges.map((edge, index) => (
              ...
              >
                {t('MainPanel.EdgeEntity', { 'entity-index': `${edge.edgeEntity.index}` })}
              </div>
            ))
          }
        </Scrollable>
    ...
    <>
      {
        [ToolState.Choosed].includes(toolState) && (
          ...
          >
            ...
            >
              {t('MainPanel.SelectedEdgeCount', { 'count': `${slectedEdgeEntities.length}` })}
            </div>
            <Button
              variant='round'
              onClick={() => {
