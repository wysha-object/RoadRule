import { updateLane, useGetLanesCmd, useGetSelectedEdgeEntityCmd, useGetSelectedLaneIndexCmd } from 'hooks/cmd'
...
function RoadRuleEditor() {
    ...
    if (mode === UIToolMode.Lane) {
        for (const lane of Object.values(masterMap).map(item => item.lanes).flat()) {
        ...
        for (const lane of Object.values(masterMap).map(item => item.masterLane)) {
            if (!selectedLaneIndex.includes(lane.laneIndex)) {
                continue
            }
    ...
    return (
        laneRulesValue === undefined ?
            <div style={{
                ...
                {mode === UIToolMode.Lane ? t('MainPanel.ChooseLane') : t('MainPanel.ChooseMasterLane')}
            </div> :
            <RulesEditor
